#include <stdio.h>
#include <stdlib.h>
#include <string.h>
/*
Write up:

Timing:

|           | 1        | 2        | 4        | 8        | 16       |
|-----------|----------|----------|----------|----------|----------|
| mutex     | 8.523851 | 4.211702 | 3.621743 | 1.610975 | 1.727959 |
| spin lock | 8.119779 | 5.953613 | 3.899753 | 1.792366 | 1.748894 |

Comments:

The difference between spin lock and mutex is that spin lock busy waits.
This means that the spin lock will occupy extra CPU usage during waiting and thus, 
in most cases, should be slower. This can been seen from results with 2 to 16 threads,
where mutex is always slightly faster. 
However, spin lock does have the benefit that the lock and unlock operation is 
relatively cheap, since we do not have extra overhead to maintain possible context switches. 
This behavior can be seen from 1 thread case: since there are no lock conflict, all operations
are run in serial, and therefore spin lock will be faster.

Furthermore, We notice that 16 threads does not give any speedup as compared to 8 threads.
This is because my machine only has 8 cores, and therefore running with 16 cores will not be 
beneficial.

*/
#include <pthread.h>
#include <assert.h>
#include <sys/time.h>

#define NUM_BUCKETS 5     // Buckets in hash table
#define NUM_KEYS 100000   // Number of keys inserted per thread
int num_threads = 1;      // Number of threads (configurable)
int keys[NUM_KEYS];

typedef struct _bucket_entry {
    int key;
    int val;
    struct _bucket_entry *next;
} bucket_entry;

typedef struct _table_entry {
    bucket_entry* list;
    pthread_spinlock_t lock;
} table_entry;

table_entry table[NUM_BUCKETS];

void panic(char *msg) {
    printf("%s\n", msg);
    exit(1);
}

double now() {
    struct timeval tv;
    gettimeofday(&tv, 0);
    return tv.tv_sec + tv.tv_usec / 1000000.0;
}

// Inserts a key-value pair into the table
void insert(int key, int val) {
    int i = key % NUM_BUCKETS;
    bucket_entry *e = (bucket_entry *) malloc(sizeof(bucket_entry));
    if (!e) panic("No memory to allocate bucket!");
    table_entry* entry = &table[i];
    pthread_spin_lock(&entry->lock);
    e->next = entry->list;
    e->key = key;
    e->val = val;
    entry->list = e;
    pthread_spin_unlock(&entry->lock);
}

// Retrieves an entry from the hash table by key
// Returns NULL if the key isn't found in the table
bucket_entry * retrieve(int key) {
    bucket_entry *b;
    for (b = table[key % NUM_BUCKETS].list; b != NULL; b = b->next) {
        if (b->key == key) return b;
    }
    return NULL;
}

void * put_phase(void *arg) {
    long tid = (long) arg;
    int key = 0;

    // If there are k threads, thread i inserts
    //      (i, i), (i+k, i), (i+k*2)
    for (key = tid ; key < NUM_KEYS; key += num_threads) {
        insert(keys[key], tid);
    }

    pthread_exit(NULL);
}

void * get_phase(void *arg) {
    long tid = (long) arg;
    int key = 0;
    long lost = 0;

    for (key = tid ; key < NUM_KEYS; key += num_threads) {
        if (retrieve(keys[key]) == NULL) lost++;
    }
    printf("[thread %ld] %ld keys lost!\n", tid, lost);

    pthread_exit((void *)lost);
}

int main(int argc, char **argv) {
    long i;
    pthread_t *threads;
    double start, end;

    if (argc != 2) {
        panic("usage: ./parallel_hashtable <num_threads>");
    }
    if ((num_threads = atoi(argv[1])) <= 0) {
        panic("must enter a valid number of threads to run");
    }

    for(i = 0; i < NUM_BUCKETS; i++) {
        pthread_spin_init(&table[i].lock, 0);
    }

    srandom(time(NULL));
    for (i = 0; i < NUM_KEYS; i++) {
        keys[i] = random();
    }

    threads = (pthread_t *) malloc(sizeof(pthread_t)*num_threads);
    if (!threads) {
        panic("out of memory allocating thread handles");
    }

    // Insert keys in parallel
    start = now();
    for (i = 0; i < num_threads; i++) {
        pthread_create(&threads[i], NULL, put_phase, (void *)i);
    }
    
    // Barrier
    for (i = 0; i < num_threads; i++) {
        pthread_join(threads[i], NULL);
    }
    end = now();
    
    printf("[main] Inserted %d keys in %f seconds\n", NUM_KEYS, end - start);
    
    // Reset the thread array
    memset(threads, 0, sizeof(pthread_t)*num_threads);

    // Retrieve keys in parallel
    start = now();
    for (i = 0; i < num_threads; i++) {
        pthread_create(&threads[i], NULL, get_phase, (void *)i);
    }

    // Collect count of lost keys
    long total_lost = 0;
    long *lost_keys = (long *) malloc(sizeof(long) * num_threads);
    for (i = 0; i < num_threads; i++) {
        pthread_join(threads[i], (void **)&lost_keys[i]);
        total_lost += lost_keys[i];
    }
    end = now();

    printf("[main] Retrieved %ld/%d keys in %f seconds\n", NUM_KEYS - total_lost, NUM_KEYS, end - start);

    return 0;
}
